...
class TestData(TestCase):
    ...
    def test_process_text_multiple(self):
        test_df = pd.DataFrame(
            [
                ["0", "@Connor @Michael Hello world", "positive"],
        ...
        self.assertTrue(
            (filtered.at[0, "text"] == "Hello world")
            and (filtered.at[1, "text"] == "Hello world two")
            and (filtered.at[2, "text"] == "Hello world three")
        )
    ...
    def test_process_sentiment_multiple(self):
        ...
        self.assertTrue(
            (filtered.at[0, "sentiment"] == [0, 0, 1.0])
            and (filtered.at[1, "sentiment"] == [1.0, 0, 0])
            and (filtered.at[2, "sentiment"] == [0, 1.0, 0])
        )
    ...
    def test_wrong_encodings(self):
        ...
        with self.assertRaises(Exception) as cm:
            data.data_preprocess(
                test_df, "text", "sentiment", 3, {"negative": 0, "neutral": 1}
            )
        except_message = str(cm.exception)
        self.assertEqual(
            except_message, "Length of label_encoding does not match number of labels"
        )
